...
//
// matrix11..matrix22 stay four parallel slices, and that was measured rather
// than assumed: folding them into one modeTransition struct — which is what
// SingleHead.Tick does, for a measured −5.3 % there — costs **+1.9 % retired
// instructions here** (3 runs each, spread < 0.3 %, `taskset -c 0 perf stat`
// over a cavity-off, nonlinearity-off Render). `&slice[i]` in place of the value
// copy made no difference. See modeTransition for why the same interleave falls
// the opposite way in the two loops. Do not "fix" the asymmetry without
// re-measuring it.
//
