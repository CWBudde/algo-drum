...
func measureDecays(hit []float64, sampleRateHz float64, options Options, partials []Partial) []Partial {
	...
	for index := range partials {
		...
		// The refinement below is fitted over the *whole* window, floor and all,
		// because in its model the floor is a parameter rather than something to
		// truncate away. It is decimated first: the envelope has already been
		// low-passed at `cutoff`, which is at most 40 Hz, so at
		// decayTraceRateHz it is oversampled several-fold and the fit runs over
		// hundreds of points rather than tens of thousands. That matters — a fit
		// run scores thousands of candidates, and an undecimated
		// Levenberg-Marquardt here would dominate the whole search.
		step := max(1, int(sampleRateHz/decayTraceRateHz))
		fullTimes := make([]float64, 0, (end-start)/step+1)
		fullTrace := make([]float64, 0, (end-start)/step+1)
...
func decayFloorFit(times, trace []float64, seedSlope, seedIntercept float64) (decayFit, bool) {
	...
	for range maxIterations {
		...
		for i, at := range times {
			modelled, gradient := decayFloorModel(at, logPower, logRate, logFloor)
...
func decayFloorModel(at, logPower, logRate, logFloor float64) (level float64, gradient [3]float64) {
	const perDecibel = 10 / math.Ln10

	signal := math.Exp(logPower - math.Exp(logRate)*at)
	...
	gradient[1] = -perDecibel * signalShare * math.Exp(logRate) * at
...
func decayFloorResidual(times, trace []float64, logPower, logRate, logFloor float64) float64 {
	...
	for i, at := range times {
		modelled, _ := decayFloorModel(at, logPower, logRate, logFloor)
