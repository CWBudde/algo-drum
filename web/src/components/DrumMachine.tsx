...
function drawScreenArea(ctx: CanvasRenderingContext2D) {
  const sx = GRID_X - 20, sy = GRID_Y - 20
  const sw = GRID_W + 40, sh = GRID_H + 40
  const r = 12
...
function drawLED(ctx: CanvasRenderingContext2D, cx: number, cy: number, r: number, color: string) {
  ctx.save()
...
function drawGrid(ctx: CanvasRenderingContext2D, pattern: boolean[][], activeStep: number) {
  ...
  for (let row = 0; row < ROWS; row++) {
    for (let col = 0; col < COLS; col++) {
      ...
      ctx.fillStyle = tg
      ctx.beginPath()
      ctx.roundRect(x + inset, y + inset, CELL_W - inset * 2, 12, [cr, cr, 0, 0])
      ctx.fill()
      ...
      ctx.fillStyle = lg
      ctx.beginPath()
      ctx.roundRect(x + inset, y + inset, 9, CELL_H - inset * 2, [cr, 0, 0, cr])
      ctx.fill()
      ...
      ctx.fillStyle = bng
      ctx.beginPath()
      ctx.roundRect(x + inset, y + CELL_H - inset - 7, CELL_W - inset * 2, 7, [0, 0, cr, cr])
      ctx.fill()
  ...
  for (let row = 0; row < ROWS; row++) {
    ...
    ctx.fillText(TRACKS[row].toUpperCase(), GRID_X - 14, GRID_Y + row * CELL_H + CELL_H / 2)
  }
  ...
  for (let col = 0; col < COLS; col++) {
    ctx.fillText(String(col + 1), GRID_X + col * CELL_W + CELL_W / 2, GRID_Y + GRID_H + 9)
  }
}
  ...
  return (
    <div ref={containerRef} style={{ width: '100%', maxWidth: 920, position: 'relative' }}>
      ...
      <div style={{
        ...
        <Knob value={tempo} onChange={setTempoState} label={`${bpm} BPM`} size={54} color="#C87828" />
        <Knob value={swing} onChange={setSwingState} label="SWING" size={54} color="#C87828" />
      </div>
