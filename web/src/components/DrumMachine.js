...
function drawPanel(ctx) {
    const x = 14, y = 14, w = CW - 28, h = CH - 28, r = 18;
...
function drawScreenArea(ctx) {
    const sx = GRID_X - 10, sy = GRID_Y - 10;
    const sw = GRID_W + 20, sh = GRID_H + 20;
    const r = 12;
...
function drawLED(ctx, cx, cy, r, color) {
    ctx.save();
...
function drawGrid(ctx, pattern, activeStep) {
    ...
    for (let row = 0; row < ROWS; row++) {
        for (let col = 0; col < COLS; col++) {
            ...
            ctx.beginPath();
            ctx.roundRect(x + inset, y + inset, CELL_W - inset * 2, CELL_H - inset * 2, cr);
            ctx.fill();
            ...
            ctx.fill();
            // Bottom inner bounce-light
            const bng = ctx.createLinearGradient(0, y + CELL_H - inset - 7, 0, y + CELL_H - inset);
    ...
    for (let row = 0; row < ROWS; row++) {
        ...
        ctx.fillText(TRACKS[row].toUpperCase(), GRID_X - 22, GRID_Y + row * CELL_H + CELL_H / 2);
    }
    // Step numbers
    ...
    for (let col = 0; col < COLS; col++) {
        ctx.fillText(String(col + 1), GRID_X + col * CELL_W + CELL_W / 2, GRID_Y + GRID_H + 19);
    }
}
...
export default function DrumMachine({ wasmLoaded }) {
    const canvasRef = useRef(null);
    const containerRef = useRef(null);
    const [pattern, setPattern] = useState(() => Array.from({ length: ROWS }, () => Array(COLS).fill(false)));
    ...
    const handleCanvasClick = useCallback((e) => {
        ...
        if (wasmLoaded)
            engine.setCell(TRACK_INDEX[row], col, next);
    ...
    const handleVolumeChange = useCallback((track, v) => {
        setVolumes((prev) => {
            const n = [...prev];
            n[track] = v;
            return n;
    ...
    const handleMuteToggle = useCallback((track) => {
        setMuted((prev) => {
            const n = [...prev];
            n[track] = !n[track];
            return n;
    ...
    return (_jsxs("div", { ref: containerRef, style: { width: "100%", maxWidth: 1020, position: "relative" }, children: [_jsxs("div", { style: {
                    ...
                    gap: 20,
                }, children: [_jsx("button", { onClick: handlePlayStop, disabled: !wasmLoaded, title: playing ? "Stop" : "Play", style: {
                            width: 48,
                            height: 48,
                            marginTop: 3,
                            borderRadius: "50%",
